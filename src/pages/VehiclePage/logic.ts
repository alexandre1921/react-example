...
function useLogic() {
  ...
  const populateInputs = useCallback((vehicleModel: IVehicle) => {
  ...
  const handleOnSubmitVehicle = useCallback((e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isAddingInputsNotNull()) {
      ...
      if (isAdding) {
        ...
        createVehiclesController.execute(newVehicle)
          .then((id) => {
        ...
        if (inputId.current !== null){
          ...
          updateVehicleController.execute(newVehicle)
            .then(() => {
              setVehicle(oldVehicles => oldVehicles
                .map(v => v.id === newVehicle.id ? {...newVehicle.toJSON()} : v)
              );
              resetForm();
            });
        }
  ...
  useEffect(() => {
    ...
    listVehiclesController.execute()
      .then(newVehicles => {
